...
data_fullpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'dataset',
                             data_filename)  # adjustable path in case script is called from another working directory
...
exec_detection = ['bev_from_pcl',
                  'detect_objects']  # options are 'bev_from_pcl', 'detect_objects', 'validate_object_labels', 'measure_detection_performance'
exec_tracking = ['perform_tracking']  # options are 'perform_tracking'
exec_visualization = [
    'show_tracks']  # options are 'show_range_image', 'show_bev', 'show_pcl', 'show_labels_in_image', 'show_objects_and_labels_in_bev', 'show_objects_in_bev_labels_in_camera', 'show_tracks', 'show_detection_performance', 'make_tracking_movie'
...
while True:
    try:
        ...
        if 'validate_object_labels' in exec_list:
            ...
            valid_label_flags = tools.validate_object_labels(frame.laser_labels, lidar_pcl, configs_det,
        ...
        if 'show_labels_in_image' in exec_list:
            img_labels = tools.project_labels_into_camera(camera_calibration, image, frame.laser_labels,
        ...
        if 'show_objects_in_bev_labels_in_camera' in exec_list:
            tools.show_objects_in_bev_labels_in_camera(detections, lidar_bev, image, frame.laser_labels,
                                                       valid_label_flags, camera_calibration, configs_det)
            cv2.waitKey(vis_pause_time)
        ...
        if 'perform_tracking' in exec_list:
            ...
            for detection in detections:
                ...
                if configs_det.lim_x[0] < detection[1] < configs_det.lim_x[1] \
                        and configs_det.lim_y[0] < detection[2] < configs_det.lim_y[1]:
